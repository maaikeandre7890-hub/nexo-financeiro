...
const AIInsightsPanel: React.FC<Props> = ({ isOpen, onClose }) => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'model', text: `Protocolo NEXO IA estabelecido em ${currentDateFormatted}. Como posso otimizar a saúde financeira da ${state.companyName || 'NEXO'} hoje?` }
  ]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          tools: [{ googleSearch: {} }],
          systemInstruction: `Você é o NEXO IA, o cérebro estratégico por trás de um SaaS financeiro. Seu tom é analítico, extremamente formal e executivo. Contexto Financeiro Atual (${currentDateFormatted}): ${financialContext}.`,
        },
      });
      ...
      setMessages(prev => [...prev, { 
        role: 'model', 
        text: response.text || "Análise indisponível no protocolo atual.",
        links: links
      }]);
    } catch (error) {
      setMessages(prev => [...prev, { role: 'model', text: "Erro crítico no processamento estratégico da inteligência." }]);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <>
      ...
      >
        <div className="flex flex-col h-full relative overflow-hidden">
          ...
          <div className="w-12 h-1 bg-white/10 rounded-full mx-auto mt-4 md:hidden shrink-0" />

          {/* HEADER PREMIUM */}
          <div className="p-6 md:p-10 border-b border-white/[0.015] flex justify-between items-center shrink-0">
            <div className="flex items-center gap-5">
              ...
              <div>
                ...
                <p className="text-[9px] font-black text-emerald-500/40 uppercase tracking-[0.4em] mt-2">Intelligence Protocol</p>
              </div>
            </div>
            <button 
              onClick={onClose} 
              className="w-12 h-12 rounded-2xl border border-white/[0.02] flex items-center justify-center text-zinc-600 hover:text-white transition-all hover:bg-white/[0.03]"
            >
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 md:p-12 space-y-10 custom-scrollbar bg-gradient-to-b from-transparent to-black/30">
               ...
               <div key={idx} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                 <div className={`max-w-[90%] md:max-w-[80%] p-6 md:p-8 rounded-[2rem] text-sm md:text-base leading-relaxed shadow-xl ${
                   msg.role === 'user' 
                    ...
                    : 'bg-white/[0.01] text-zinc-300 border border-white/[0.02] rounded-tl-none font-medium'
                 }`}>
                   {msg.text}
                   {msg.links && msg.links.length > 0 && (
                     <div className="mt-8 pt-6 border-t border-white/[0.01] space-y-3">
                       <p className="text-[9px] font-black uppercase tracking-[0.3em] text-emerald-500/60 mb-4">Referências Consultadas</p>
                       {msg.links.map((link, i) => (
                         <a key={i} href={link.uri} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 group/link p-2 -ml-2 rounded-xl hover:bg-white/[0.02] transition-all">
             ...
             {isLoading && (
               <div className="flex items-center gap-4 animate-pulse px-2">
                  ...
                  <span className="text-[10px] font-black text-emerald-500/60 uppercase tracking-[0.5em]">Processando...</span>
               </div>
             )}
          </div>

          {/* INPUT FOOTER */}
          <div className="p-6 md:p-10 border-t border-white/[0.015] bg-[#000000] shrink-0">
            <div className="flex items-center gap-4 p-2 bg-white/[0.01] rounded-[2rem] border border-white/[0.03] focus-within:border-emerald-500/20 transition-all">
              <input 
                type="text" 
                placeholder="Questione sua estratégia..." 
