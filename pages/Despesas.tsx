    ...
    <div className="space-y-10 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="glass-card p-8 rounded-[2rem] border-white/5">
           <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2">Categorias Dominantes</p>
           <div className="flex gap-4">
              <span className="px-3 py-1 bg-slate-900 border border-slate-800 rounded-lg text-[10px] font-black text-slate-400">INFRA</span>
              <span className="px-3 py-1 bg-slate-900 border border-slate-800 rounded-lg text-[10px] font-black text-slate-400">FIXO</span>
           </div>
        </div>
      </div>
            ...
            {state.expenses.map(exp => (
              <tr key={exp.id} className="hover:bg-rose-500/[0.01] transition-colors">
                ...
                <td className="px-8 py-6">
                   <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-2 py-1 bg-slate-900 rounded">{exp.category}</span>
                </td>
                <td className="px-8 py-6 text-slate-400 text-xs font-medium">{new Date(exp.date).toLocaleDateString('pt-BR')}</td>
